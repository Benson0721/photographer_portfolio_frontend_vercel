...
const setupObserver = async () => {
  let attempts = 0;
  const maxAttempts = 5;
  const expectedImages = galleryStore.galleryImages.length;
...
<template>
  ...
  <VueEasyLightbox
    v-if="galleryStore.galleryImages.length > 0"
    :visible="visible"
    :imgs="galleryStore.galleryImages.map((item) => item.imageURL)"
    :index="index"
    @hide="hideLightbox"
  />
</template>
